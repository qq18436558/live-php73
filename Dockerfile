...
RUN apt update && DEBIAN_FRONTEND=noninteractive apt-get install -y wget tzdata && \
    ...
    wget https://github.com/qq18436558/livezilla/archive/8.0.2.2.tar.gz -O /tmp/livezilla-8.0.2.2.tar.gz && \
    tar zxvf /tmp/livezilla-8.0.2.2.tar.gz -C /tmp/ && \
    ...
    mkdir -p /run/php /logs /etc/pki/tls/certs/ /var/www/html && \
    rm -rf /var/www/html && mv /tmp/livezilla-8.0.2.2 /var/www/html && chown -R www-data:www-data /var/www/html && \
